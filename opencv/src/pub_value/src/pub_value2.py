...
def callback(msg):
  ...
  #rospy.loginfo("deg : %0.3f" %msg.ranges[0])
    global deg0, deg30_float, deg30_str, deg40, deg45, deg78, deg90_float, deg90_str, deg270_float, deg270_str, deg282, deg300, deg315, deg320, deg330_float, deg330_str, send_value
...
#        #print("dump")
    ...
    if msg.ranges[300] <= 8.000:
        deg300=msg.ranges[300]
        #print("^_________________________________^")
    elif msg.ranges[300] >= 8.000 and msg.ranges[301] <= 8.000:
        deg300=msg.ranges[301]
        #print("^_________________________________^")
    elif msg.ranges[300] >= 8.000 and msg.ranges[301] >= 8.000 and msg.ranges[299] <= 8.000:
        deg300=msg.ranges[299]
...
######################################first########################################
    ...
    while not rospy.is_shutdown():
        if sw == 0:
            ...
            if (pub_log[j] == "right") and (deg270_float <= 1.000):
                pub.publish("back")
            #elif ((pub_log[j] == "back") or (pub_log[j] == "left_slow") or (pub_log[j] == "right_slow")) and (deg270_float >= 0.900):
                #pub.publish("right_slow")
            elif (pub_log[j] == "back_slow"):
                pub.publish(pub_log[j])
            #elif (pub_log[j] == "back_slow") and (deg270_float > 0.500) and (deg270_float <= 0.900):
                #pub.publish("left_slow")
            elif (pub_log[j] == "right") and (deg90_float > 0.500) and ((deg90_float+deg270_float) < 2.200):
                pub.publish("back")
            elif (j > (i-50)) and (pub_log[j] == "back") and (deg270_float > 1.000):
                pub.publish("right")
            elif (j > (i-50)) and (pub_log[j] == "back") and (deg270_float <= 1.000):
                pub.publish("back")
            else:
            ...
            #    pub.publish("back")
            if ((i == j) or ((deg0 < 8.000) and (deg0 > 2.200))):
                sw = 3
        elif (sw == 3):
